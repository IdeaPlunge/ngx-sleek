...
const config = {
  libraryName: 'ngx-sleek',
  unscopedLibraryName: 'ngx-sleek',
  allSrc: 'src/**/*',
  allTs: 'src/**/!(*.spec).ts',
...
gulp.task('build:demo', () => {
  return execDemoCmd(`build --preserve-symlinks --prod --base-href /ngx-sleek/ --deploy-url /ngx-sleek/`, { cwd: `${config.demoDir}`});
});
...
gulp.task('build:demo-ssr', () => {
  return execDemoCmd(`build --preserve-symlinks --prod`, { cwd: `${config.demoDir}`})
    .then(() => execDemoCmd(`run ngx-sleek-demo:server`, { cwd: `${config.demoDir}` }))
